...
class VerifyContent
  ...
  attr_reader :is_error, :error_message, :list_project
      ...
      if !@init_config.key?('scans')
        ...
        for x in @init_config['scans']
          ...
          if !x.key?('project')
            ...
            unless x['project'].key?('directory')
              @is_error = true
              @error_message = 'In list of project the `directory` is missing'
              break
            end
            unless x['project']['directory'].key?('include')
              @is_error = true
              @error_message = 'In list of project the `include` is missing'
              break
            end
            unless x['project']['directory']['include'].instance_of? Array
              @is_error = true
            ...
            for lang in x['project']['language']
              next if LangugeExt.lang_ext_list.key?(lang.to_sym)

              @is_error = true
              @error_message = "`#{lang}` is not acceptable language"
              break
            end

            if x['project']['directory'].key?('exclude') && !(x['project']['directory']['exclude'].instance_of? Array)
              @is_error = true
